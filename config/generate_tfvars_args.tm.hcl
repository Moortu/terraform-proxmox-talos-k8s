# Generate TF_CLI_ARGS for all stacks to automatically load tfvars files
# This eliminates the need to duplicate this configuration in every stack.tm.hcl
generate_hcl "_generated_tfvars_args.tm.hcl" {
  condition = tm_can(global.environment) && tm_can(global.stack_name)
